...
simu_ewma2 <- function(oh_lc, lambda_1=0.25, lambda_2=0.05, win_dow=51) {
  # calculate EWMA prices
  weights_1 <- exp(-lambda_1*1:win_dow)
  weights_1 <- weights_1/sum(weights_1)
  weights_2 <- exp(-lambda_2*1:win_dow)
  ...
  ewma_1[1:(win_dow-1)] <- ewma_1[win_dow]
  ewma_2 <- filter(cl_ose, filter=weights_2, sides=1)
  ewma_2[1:(win_dow-1)] <- ewma_2[win_dow]
