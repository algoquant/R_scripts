...
List lm_arma(const arma::colvec& res_ponse, const arma::mat& ex_plain) {
  // add intercept column to explanatory matrix
  arma::mat explain_plus = join_rows(ones(ex_plain.n_rows), ex_plain);
  int num_rows = ex_plain.n_rows, num_cols = explain_plus.n_cols;
  int deg_free = (num_rows - num_cols);
  
  // fit the model res_ponse ~ ex_plain, and calculate beta coefficients
  arma::colvec beta_s = arma::solve(explain_plus, res_ponse);
  // calculate residuals
  arma::colvec resid_uals = res_ponse - explain_plus*beta_s;
  ...
  NumericVector stat_s(2);
  stat_s(0) = r_squared;
  stat_s(1) = f_stat;
  stat_s.attr("names") = Rcpp::CharacterVector::create("R-squared", "F-statistic");

  // calculate standard errors of beta coefficients
  arma::colvec std_err = arma::sqrt(res_sumsq/deg_free*arma::diagvec(arma::pinv(arma::trans(explain_plus)*explain_plus)));
  // calculate t-values and p-values of beta coefficients
  arma::colvec beta_tvals = beta_s/std_err;
  arma::colvec beta_pvals = 2*Rcpp::pt(-abs(wrap(beta_tvals)), deg_free);
  NumericMatrix co_ef = wrap(join_rows(join_rows(join_rows(beta_s, std_err), beta_tvals), beta_pvals));
