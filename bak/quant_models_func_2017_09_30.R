...
### Function load_data() loads time series data from csv files,
## and cbinds them together into one xts time series.
...
hedge_ratio <- function(prices_ts_x, prices_ts_y,
  ...
  if (model_type == 0) {
    ...
    return(list(reg_summary = summary(reg_model),
                ...
                port_price_mean = mean(prices_ts_y - hedge_ratio * prices_ts_x),
                port_price_sd = sd(prices_ts_y - hedge_ratio * prices_ts_x)))
