...
data(edhec)
edhec <- read.csv('C:/Devel/R/R-2.15.0/library/PerformanceAnalytics/data/edhec.csv',stringsAsFactors=FALSE) #my local update of this data
edhec  <- edhec[,1:(ncol(edhec)-1)]
edhec  <- xts(edhec[,-1],order.by=as.POSIXct(edhec[,1]))
# data(factors)
...
chart.RiskReturnScatter(edhec.R,add.sharpe=c(1,2,3),main="EDHEC Index Since Inception Performance", colorset=rainbow8equal, ylim=c(0,.2), xlim=c(0,.12))
dev.off()
...
M <- cor(edhec.R)
colnames(M) = rownames(M) = colnames(edhec.R)
...
colnames(M36) = rownames(M36) = colnames(edhec.R)
