...
oh_lc <- get(sym_bol, data_env)
clo_se <- log(quantmod::Cl(oh_lc))
perf_stats <- lapply(4:15, backtest_ewma, oh_lc=oh_lc, lagg=lagg, thresh_old=thresh_old, co_eff=co_eff)
position_s <- lapply(perf_stats, function(x_ts) {
x_ts["2018-01-01/2020-04-22", "positions"]
})  # end lapply
position_s <- do.call(cbind, position_s)
...
oh_lc <- get(sym_bol, data_env)
clo_se <- log(quantmod::Cl(oh_lc))
perf_stats <- lapply(14:17, backtest_ewma, oh_lc=oh_lc, lagg=lagg, thresh_old=thresh_old, co_eff=co_eff)
position_s <- lapply(perf_stats, function(x_ts) {
x_ts["2018-01-01/2020-04-22", "positions"]
})  # end lapply
position_s <- do.call(cbind, position_s)
...
oh_lc <- get(sym_bol, data_env)
clo_se <- log(quantmod::Cl(oh_lc))
perf_stats <- lapply(2:10, backtest_ewma, oh_lc=oh_lc, lagg=lagg, thresh_old=thresh_old, co_eff=co_eff)
position_s <- lapply(perf_stats, function(x_ts) {
  x_ts["2018-01-01/2020-04-22", "positions"]
})  # end lapply
position_s <- do.call(cbind, position_s)
