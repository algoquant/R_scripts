...
#' @export
chart.xts <- function(ts.data, name.plot=NULL, ts.indeks=NULL) {
  stopifnot(inherits(ts.data, "xts"))
  theme <- chart_theme()
#  if(name=='Signals') theme$col$line.col = 'red' 

  par(mfrow=c(dim(ts.data)[2],1))
  sapply(1:(dim(ts.data)[2]), function(n.col)
         {
           invisible(chart_Series(ts.data[,n.col], name=paste(colnames(ts.data[,n.col]), "/", date())))
...
#' @export
chart.multi.xts <- function(ts.prices) {
  par(mfrow=c(dim(ts.prices)[2],1))
  sapply(1:(dim(ts.prices)[2]), function(n.col) chart.xts(ts.prices[,n.col], name.plot=paste(colnames(ts.prices[,n.col]), "/", date())))
...
### Plot a price time series and its SavGol estimates using chart.xts
#' @export
...
# Shade the plot and show Signals and Positions only if the number of sign flips is small
  ...
  chart.xts(chart.data, ts.indeks=ts.indeks)
}
### End chart.SavGol
